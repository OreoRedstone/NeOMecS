...
namespace NeOMecS.Utilities
{
    public static class SaveLoadSystem
    {
        ...
        public static SimState Decode(string text)
        {
            ...
            state.simSpeed = Convert.ToDouble(Regex.Match(text, @"<simSpeed>(.*?)<simSpeed/>").Groups[0].Value);
            state.universe.gravitationalConstant = Convert.ToDouble(Regex.Match(text, @"<gravitationalConstant>(.*?)<gravitationalConstant/>").Groups[0].Value);
            var cameraPos = Regex.Match(text, @"<gravitationalConstant>(.*?), (.*?)<gravitationalConstant/>");
            state.cameraPosition.x = Convert.ToDouble(cameraPos.Groups[0].Value);
            state.cameraPosition.y = Convert.ToDouble(cameraPos.Groups[1].Value);
            ...
            foreach (Match match in Regex.Matches(text, @"<body><name>(.*?)<name/><radius>(.*?)<radius/><colour>(.*?), (.*?), (.*?)<colour/><position>(.*?), (.*?)<position/><velocity>(.*?), (.*?)<velocity/><mass>(.*?)<mass/><body/>").ToList<Match>())
            {
                ...
                var radius = Convert.ToDouble(match.Groups[1].Value);
                var colour = new Colour(Convert.ToInt32(match.Groups[2].Value), Convert.ToInt32(match.Groups[3].Value), Convert.ToInt32(match.Groups[4].Value));
